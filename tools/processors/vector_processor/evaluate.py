...
def evaluate(filename, lang="en"):
    """Compute Spearman rank coefficient for each evaluation file"""
    # Set evaluation directory based on language
    eval_dir = os.path.join(BASE_EVAL_DIR, lang)
...
def stats(lang="en"):
    ...
    print(f"\nEvaluation results for {lang.upper()} datasets:")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lang', type=str, default="en", choices=["en", "fr"],
                        help='Language to evaluate (en or fr)')

    args = parser.parse_args()
    
    # Reset global variables for each evaluation
    results = dict()
    missed_pairs = dict()
    missed_words = dict()
    
    # Set evaluation directory based on language
    eval_dir = os.path.join(BASE_EVAL_DIR, args.lang)
    
    # Check if evaluation directory exists
    if not os.path.exists(eval_dir):
        print(f"Error: Evaluation directory for {args.lang} not found at {eval_dir}")
        sys.exit(1)
        
    init_results(eval_dir)
    for f in args.filenames:
        evaluate(f, args.lang)
    stats(args.lang)
